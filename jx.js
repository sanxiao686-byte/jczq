...
(function(){
  ...
  function mergeExtra(arr){
    if(!arr||!DATA.matches) return;
    arr.forEach(function(ex){
      const m=DATA.matches.find(function(x){return x.code===ex.code;});
      if(!m) return;
  ...
  Promise.all([0,1,2].map(function(i){
    return fetch('./e'+i+'.json',{cache:'no-store'}).then(function(r){return r.ok?r.json():[];}).catch(function(){return [];});
  })).then(function(parts){
    window._EXTRAS=parts.reduce(function(a,b){return a.concat(b||[]);},[]);
    mergeExtra(window._EXTRAS);
    if(DATA.matches&&DATA.matches.length) render();
  });
  const _render=render;
  render=function(){ if(window._EXTRAS) mergeExtra(window._EXTRAS); _render(); };
})();
