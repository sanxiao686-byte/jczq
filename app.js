...
function boot() {
  ...
  Promise.all([0, 1, 2].map(function (i) {
    return fetch("./m" + i + ".json?v=20", { cache: "no-store" }).then(function (r) { return r.ok ? r.json() : null; }).catch(function () { return null; });
  })).then(function (parts) {
