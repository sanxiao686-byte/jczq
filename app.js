...
function euroBlock(m) {
  fillOdds(m);
  var rows = m.euroRows || [];
  if (!rows.length) return '<div class="sect"><h3>欧盘</h3><div class="muted">本场尚未抓到欧赔公司盘（竞彩官方见上栏）。来源：500网即时欧赔 XML。足彩网指数页当前 403，未混入推算值。</div></div>';
  return '<div class="sect"><h3>欧盘 + 凯利 <span class="muted">500网即时XML 2026-09-12 15:33 · 初盘未公布故留空</span></h3><table class="odds-tb"><thead><tr><th>公司</th><th>初盘 胜/平/负</th><th>即时 胜/平/负</th><th>凯利 胜/平/负</th><th>来源</th></tr></thead><tbody>' +
    rows.map(function (r) {
...
function ahBlock(m) {
  ...
  if (m.ahRows && m.ahRows.length) {
    html += '<div class="sect"><h3>亚盘让球 <span class="muted">500网即时亚盘 · 初盘未公布故留空</span></h3><table class="odds-tb"><thead><tr><th>公司</th><th>初盘</th><th>即时</th></tr></thead><tbody>' +
...
function openBase(code) {
  ...
  w.document.write("<!DOCTYPE html><html><head><meta charset=utf-8><title>base</title></head><body>"+content+"</body></html>"); w.document.close();
}
function jxBtn(code) {
  return '<a class="jx" href="javascript:void(0)" onclick="event.stopPropagation();openBase(\'' + code + '\')">析</a>';
}
...
function boot() {
  ...
  Promise.all([0, 1, 2].map(function (i) {
    return fetch("./m" + i + ".json?v=38", { cache: "no-store" }).then(function (r) {
      if (!r.ok) return null;
